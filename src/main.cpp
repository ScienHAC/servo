...
boolean detectObject()
{
  ...
  // Fixing color detection - using threshold values that can be calibrated
  // Lower values mean more of that color is detected
  const int threshold = 50;

  // Check if any valid reading is obtained
  if (redFreq < threshold || greenFreq < threshold || blueFreq < threshold)
  {
    // Find the minimum frequency - that's our dominant color
    int minFreq = min(redFreq, min(greenFreq, blueFreq));

    // Determine which color has the minimum frequency
    if (minFreq == redFreq && redFreq < greenFreq * 0.8 && redFreq < blueFreq * 0.8)
    {
      Serial.println("Detected RED object");
      detectedColor = "red";
      targetBasePosition = baseRedPos;
    }
    else if (minFreq == greenFreq && greenFreq < redFreq * 0.8 && greenFreq < blueFreq * 0.8)
    {
      Serial.println("Detected GREEN object");
      detectedColor = "green";
      targetBasePosition = baseGreenPos;
    }
    else if (minFreq == blueFreq && blueFreq < redFreq * 0.8 && blueFreq < greenFreq * 0.8)
    {
    ...
    {
      Serial.println("Unknown colored object");
...
void pickUpObject()
{
  ...
  // 5. Check if object is detected and identify its color
  Serial.println("Checking for object...");
  objectDetected = detectObject();

  if (!objectDetected)
  {
    Serial.println("No object detected. Returning to start position.");
  ...
  // 6. Close grabber to grab object
  ...
  // 7. Detach grabber servo to prevent overheating and servo strain while holding
  Serial.println("Detaching grabber servo to prevent overheating");
  grabberServo2.detach();
  delay(1000);

  // 8. Move arm back to middle position
  Serial.println("Moving arm to middle position");
  moveServoGradually(armServo2, armPickPos, armMidPos, 15);
  delay(1000);

  // 9. Move arm to rest position (0)
  Serial.println("Moving arm to rest position");
  moveServoGradually(armServo2, armMidPos, armRestPos, 15);
  delay(1000);

  // 10. Move joint to lifting position (0) using gradual movement
...
void releaseObject()
{
  // Only proceed if an object was detected and grabbed
  if (!objectDetected)
  {
    Serial.println("No object to release. Skipping release sequence.");
    return;
  }
...
void loop()
{
  ...
  // Only release if we actually picked something up
  if (objectDetected)
  {
    releaseObject();
    delay(1000);
  ...
  {
    // If no object was detected, wait a bit before trying again
    Serial.println("No object detected - waiting before trying again");
    delay(2000);
  }
}